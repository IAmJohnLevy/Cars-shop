...
export class FilterDataService {
  ...
  getFilteredCars(): Observable<Icar[]> {
    return new Observable<Icar[]>(subscriber => {
      const subscription = this.filterEnabled$.subscribe(filterEnabled => {
        const subscription1 = this.filter1Enabled$.subscribe(filter1Enabled => {
          const subscription2 = this.filter2Enabled$.subscribe(filter2Enabled => {
            let filteredCars = [...this.allCars];
            if (filterEnabled) {
              if (filter1Enabled) {
                filteredCars = filteredCars.filter(car => car.bodystyle === 'SUV');
              }
              if (filter2Enabled) {
                filteredCars = filteredCars.filter(car => car.bodystyle === '2D Chassis');
              }
            }
